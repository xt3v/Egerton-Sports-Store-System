...
public class BorrowedItemRepository implements Repository<Integer,BorrowedItem> {
    ...
    public Optional<BorrowedItem> getById(Integer id) {
        ...
        try{
            ...
            if(rs.next()){
                borrowedItem = new BorrowedItem(rs.getInt("itemId"), rs.getDate("borrowedDate").toLocalDate(),rs.getInt("borrowedItemId"),rs.getInt("quantity"),rs.getString("borrowerId"));
            }
    ...
    private ArrayList<BorrowedItem> createList(ResultSet rs) {
        ...
        try{
            while (rs.next()){
               BorrowedItem borrowedItem = new BorrowedItem(rs.getInt("itemId"), rs.getDate("borrowedDate").toLocalDate(),rs.getInt("borrowedItemId"),rs.getInt("quantity"),rs.getString("borrowerId"));
  ...
  public ArrayList<BorrowedItem> getByBorrowerId(String id) {
   ...
   public ArrayList<BorrowedItem> getByItemId(int id){
       try{
           String sql = "SELECT * FROM borroweditems WHERE itemId = ?";
           PreparedStatement statement = db.getConn().prepareStatement(sql);
           statement.setInt(1,id);
           return createList(statement.executeQuery());
       } catch (SQLException e) {
           System.out.println(e.getMessage() + "get bowworeditems by item reg");
       }
       return new ArrayList<>();
   }
}
