...
import TableDetails.DebtorDetails;
...
public class ViewDebtorsController implements Initializable {
    @FXML TableView<DebtorDetails> debtorsTable;
    @FXML TableColumn<DebtorDetails,String> idColumn;
    @FXML TableColumn<DebtorDetails,String> nameColumn;
    @FXML TableColumn<DebtorDetails,String> phoneColumn;
    @FXML TableColumn<DebtorDetails,String> balanceColumn;

    private ObservableList<DebtorDetails> data; //store all the debtros colmns
   ...
   @FXML public void reduceDebt(ActionEvent actionEvent) {
        DebtorDetails debtor = debtorsTable.getSelectionModel().getSelectedItem(); //get selected column
    ...
    @FXML void populateTable() {
        ...
        for(Customer c : list){
            DebtorDetails detail = new DebtorDetails(String.format("%d",c.getId_no()),c.getName(),c.getPhone(),String.format("%.3f",c.getBalance()));
            data.add(detail);
        }
    }
}
