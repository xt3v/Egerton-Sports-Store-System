...
public class SaleDAO implements DAO {
    ...
    public Object getByIdentifier(Object identifier) {
        ...
        try{
            ...
            if(rs.next()){
                ...
                sale = new Sale(rs.getDate("date"),ray,rs.getTime("time"),rs.getFloat("total"),rs.getFloat("credited"),true);
    ...
    public ArrayList getAll() {
        ...
        try{
            ...
            while (rs.next()){
                JsonArray ray = (JsonArray) new JsonParser().parse(rs.getString("products"));
                Sale sale = new Sale(rs.getDate("date"),ray,rs.getTime("time"),rs.getFloat("total"),rs.getFloat("credited"),true);
                sale.setSale_id(rs.getString("sale_id"));
                list.add(sale);
            }
        } catch (SQLException e) {
    ...
    public ArrayList queryList(String sql) {
        ...
        try{
            ...
            while (rs.next()){
                JsonArray ray = (JsonArray) new JsonParser().parse(rs.getString("products"));
                Sale sale = new Sale(rs.getString("date"),ray,rs.getTime("time"),rs.getFloat("total"),rs.getFloat("credited"),true);
                sale.setSale_id(rs.getString("sale_id"));
                list.add(sale);
            }
        } catch (SQLException e) {
    ...
    public boolean save(Object entry) {
        ...
        String date = sale.getDate().toGMTString();    //convert date to string
